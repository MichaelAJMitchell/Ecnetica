...
class BKTConfig:
    ...
    #eg if 0.5 then the mastery decays for the first connected node by 1/2 and the second by a 1/4
    area_effect_min_effect: float = 0.01   # Minimum effect to apply


@dataclass
class AlgorithmConfig:
    """Configuration class for mcq algorithm weights and parameters"""
    #Greedy Selection Algorithm Parameters
    greedy_priority_weight: float = 1.5         # Weight for low mastery priority
    greedy_subtopic_weight: float = 0.7          # Weight for subtopic coverage
    greedy_prereq_weight: float = 0.5            # Weight for prerequisite coverage
    greedy_difficulty_penalty: float = 1.5       # Penalty multiplier for difficulty mismatch
    greedy_too_easy_penalty: float = 1.5        # Extra penalty for too-easy questions
    greedy_importance_weight: float = 0.3        # Weight for topic importance bonus
    greedy_mastery_update_rate: float = 1     # How much virtual mastery increases per question
    greedy_convergence_threshold: float = 0.05   # Minimum improvement to continue iteration

    # Performance optimization parameters
    greedy_max_mcqs_to_evaluate: int = 100       # Limit MCQs evaluated per iteration
    greedy_early_stopping: bool = False         # Stop when improvement is minimal



    # Difficulty Parameters
    difficulty_range: float = 0.5  # How far from student mastery to allow
    mastery_threshold: float = 0.7  # Threshold for considering topic "due"
    chapter_mastery_threshold: float = 0.9  # Higher threshold for chapter reviews

    # Confidence-based difficulty matching parameters
    confidence_tolerance_factor: float = 0.5  # How much confidence affects difficulty tolerance (0.0 to 1.0)

    # Confidence Parameters
    min_confidence: float = 0.1 #to avoid extreme cases
    max_confidence: float = 0.95
    confidence_growth_rate: float = 0.02
    attempts_for_full_confidence: int = 10  # Attempts needed for topic confidence

    # Base Confidence Calculation Weights
    confidence_question_weight: float = 0.3  # Weight for number of questions attempted
    confidence_time_weight: float = 0.3      # Weight for time spent on system
    confidence_consistency_weight: float = 0.2  # Weight for usage consistency

    # Base Confidence Scaling Parameters
    questions_for_full_factor: int = 100     # Questions needed for max question factor
    hours_for_full_factor: float = 20.0      # Hours needed for max time factor
    sessions_for_full_factor: int = 50       # Sessions needed for max session factor

    # Topic Confidence Weights
    topic_attempt_weight: float = 0.7        # Weight for attempts on specific topic
    topic_decay_weight: float = 0.3          # Weight for time decay factor

    # Time Parameters
    confidence_decay_halflife: float = 30.0  # Days for confidence to halve
...
class StudentProfile:
    ...
    # Add BKT-related tracking
    bkt_parameters: Dict[int, 'BKTParameters'] = field(default_factory=dict)  # Custom params per topic
    ...
    def calculate_base_confidence(self, config: AlgorithmConfig = None) -> float:
        ...
        question_factor = min(1.0, self.total_questions_attempted / config.questions_for_full_factor)

        # Time factor (0 to 1)
        time_factor = min(1.0, (self.total_time_on_system / 60.0) / config.hours_for_full_factor)
        ...
        base_confidence = (
            config.confidence_question_weight * question_factor +
            config.confidence_time_weight * time_factor +
            config.confidence_consistency_weight * consistency_factor
        )

        # Scale to confidence range
        return self.min_confidence + base_confidence * (self.max_confidence - self.min_confidence)

    def calculate_topic_confidence(self, node_index: int, kg, config: AlgorithmConfig = None) -> float:
        """Calculate confidence for a specific topic"""
        if config is None:
            config = AlgorithmConfig()

        base_confidence = self.calculate_base_confidence(config)
        ...
        attempt_factor = min(1.0, len(attempts) / config.attempts_for_full_confidence)
        ...
        decay_factor = 0.5 ** (days_since_attempt / config.confidence_decay_halflife)

        # Combine factors
        topic_confidence = (
            config.topic_attempt_weight * attempt_factor +
            config.topic_decay_weight * decay_factor
        )

        # Blend with base confidence
        final_confidence = base_confidence + topic_confidence * (self.max_confidence - base_confidence)

        return min(self.max_confidence, max(self.min_confidence, final_confidence))
    ...
    def set_ability_level(self, ability_name: str, level: float):
        ...
        if ability_name not in ['problem_solving', 'memory', 'notation', 'algebra', 'interconnectedness']:
...
class MCQScheduler:
    ...
    def __init__(self, knowledge_graph, student_manager, config: AlgorithmConfig = None):
    ...
    def get_eligible_mcqs_for_greedy_selection(self, student_id: str) -> List[str]:
        ...
        for mcq_id, mcq_vector in self.mcq_vectors.items():
            ...
            if main_topic_mastery >= self.config.mastery_threshold:
    ...
    def select_mcqs_greedy(self, student_id: str, num_questions: int = 1,
        ...
        if len(eligible_mcqs) > self.config.greedy_max_mcqs_to_evaluate:
        ...
        if not topic_priorities:
            ...
            return self._select_non_due_topics(eligible_mcqs, student_id, num_questions, use_chapter_weights)
        ...
        for iteration in range(num_questions):
            ...
            if (self.config.greedy_early_stopping and abs(total_coverage - last_total_coverage) < self.config.greedy_convergence_threshold):
    ...
    def _quick_priority_score(self, mcq_id: str, student: StudentProfile) -> float:
        ...
        for main_topic_index, weight in mcq_vector.subtopic_weights.items():
            ...
            topic_need = (1.0 - mastery) ** self.config.greedy_priority_weight
        ...
        for main_topic_index in student.studied_topics:
            if student.is_topic_studied(main_topic_index):
                ...
                if mastery < self.config.mastery_threshold:
                    ...
                    priority = (1.0 - mastery) * self.config.greedy_priority_weight
    ...
    def _calculate_weighted_coverage(self, mcq_vector: MCQVector,topic_priorities: Dict[int, float],virtual_mastery: Dict[int, float]) -> Dict:
        ...
        for main_topic_index, mcq_weight in mcq_vector.subtopic_weights.items():
            if main_topic_index in topic_priorities: 
                ...
                if main_topic_index == mcq_vector.primary_main_topic_index:
                    ...
                    coverage = mcq_weight * topic_priority * self.config.greedy_subtopic_weight
        ...
        for prereq_index, prereq_weight in mcq_vector.prerequisites.items():
            if prereq_index in topic_priorities: 
                ...
                coverage = prereq_weight * topic_priority * self.config.greedy_prereq_weight
    ...
    def _update_virtual_mastery_and_priorities(self, mcq_id: str,
        ...
        for main_topic_index, topic_weight in mcq_vector.subtopic_weights.items():
            if main_topic_index in topic_priorities:  # Only update due topics
                ...
                if main_topic_index == mcq_vector.primary_main_topic_index:
                    ...
                    mastery_increase = mcq_vector.difficulty * self.config.greedy_mastery_update_rate
                else:
                    # Subtopics get weighted boost
                    mastery_increase = (mcq_vector.difficulty * topic_weight * self.config.greedy_mastery_update_rate)
                ...
                if new_mastery >= self.config.mastery_threshold:
                    ...
                    new_priority = (1.0 - new_mastery) ** self.config.greedy_priority_weight
        ...
        for prereq_index, prereq_weight in mcq_vector.prerequisites.items():
            if prereq_index in topic_priorities:  # Only update due prerequisites
                ...
                mastery_increase = (mcq_vector.difficulty * prereq_weight * self.config.greedy_mastery_update_rate * 0.5)
    ...
    def _calculate_difficulty_cost_enhanced(self, mcq_vector: MCQVector,virtual_mastery: Dict[int, float],student: StudentProfile) -> float:
        ...
        if mcq_vector.difficulty < weighted_mastery:
            ...
            difficulty_cost = difficulty_diff * self.config.greedy_too_easy_penalty
        else:
            # penalty for difficulty mismatch
            difficulty_cost = difficulty_diff * self.config.greedy_difficulty_penalty
    ...
    def _calculate_importance_bonus_enhanced(self, mcq_vector: MCQVector,topic_priorities: Dict[int, float]) -> float:
        ...
        for main_topic_index, mcq_weight in mcq_vector.subtopic_weights.items():
            if main_topic_index in topic_priorities:
                ...
                topic_importance = out_degree * mcq_weight * self.config.greedy_importance_weight
...
class BayesianKnowledgeTracing:
    ...
    def __init__(self, knowledge_graph, student_manager,default_params: BKTParameters = None, config: BKTConfig = None,scheduler: Optional[MCQScheduler] = None):
        """
    ...
    def calculate_conditional_probability(self, current_mastery: float, is_correct: bool, params: BKTParameters) -> float:
        """
    ...
    def process_mcq_response_improved(self, student_id: str, mcq_id: str,
        ...
        # Try to get MCQ vector for proper weights
        # Import MCQScheduler here to avoid circular imports
        #from your_module import MCQScheduler  # Replace with actual import
    ...
    def apply_area_of_effect(self, student_id: str, center_main_topic_index: int,mastery_change: float, max_distance: int = 3,decay_rate: float = 0.5) -> List[Dict]:
        """
    ...
    def calibrate_parameters(self, student_id: str, main_topic_index: int,attempt_history: List[Tuple[bool, datetime]]) -> BKTParameters:
        """
        ...
        return BKTParameters(prior_knowledge=adjusted_prior,learning_rate=adjusted_learning,slip_rate=max(0.01, min(0.3, adjusted_slip)),guess_rate=current_params.guess_rate  )



def load_comprehensive_mcqs(kg):
    """
    Load all predefined MCQs into the knowledge graph.
    the commenting is all weird on this ignore it
    """
    mcq_ids = []
    mcqs = []
    # QUESTION 1 VARIATIONS: Basic quadratic form identification

    # Version 1A: Minimal difficulty
    mcqs.append(kg.create_mcqs(
        text='Which equation shows a quadratic in standard form?',
        options=[
            'y = 2x + 1',
            'x² + 3x - 4 = 0',
            'y = x² + 2x + 1',
        ...
        correctindex=1,
        option_explanations=[
            'This is a linear equation in slope-intercept form (y = mx + b), not a quadratic. Quadratic equations must have an x² term and equal zero.',
            'Correct! This follows the standard form ax² + bx + c = 0 where a=1, b=3, c=-4. The equation is set equal to zero with all terms on one side.',
            'This is a quadratic equation but in function form (y = ax² + bx + c), not standard form. Standard form must equal zero.',
            'This is a linear equation with only x to the first power, lacking the x² term required for quadratics.'
        ],
        main_topic_index=2,
        subtopic_weights={2: 1.0},
        conceptual=0.1, procedural=0.0, problem_solving=0.1, communication=0.2, memory=0.3, spatial=0.0
    ))

    # Version 1B: Medium difficulty
    mcqs.append(kg.create_mcqs(
    ...
    # Version 1C: High difficulty
    mcqs.append(kg.create_mcqs(
        text='A student writes \\(3x^2 = 7x - 2\\). What form must this be rewritten in to identify it as a quadratic equation in standard form?',
        options=[
            '\\(3x^2 - 7x + 2 = 0\\)',
            '\\(x^2 = \\frac{7x - 2}{3}\\)',
            '\\(3x^2 + 7x - 2 = 0\\)',
        ...
        correctindex=0,
        option_explanations=[
            'Correct! Moving all terms to one side gives \\(3x^2 - 7x + 2 = 0\\), which is standard form ax² + bx + c = 0 with a=3, b=-7, c=2.',
            'This isolates x² but is not standard form. Standard form requires all terms on one side equal to zero, not solving for one variable.',
            'This has the wrong sign for the middle term. When moving 7x from the right side, it becomes -7x on the left: \\(3x^2 - 7x + 2 = 0\\).',
            'This is function notation (y = ...), not standard form. Standard form sets the expression equal to zero, not to a dependent variable.'
        ],
        main_topic_index=2,
        subtopic_weights={2: 0.8, 1: 0.2},
        conceptual=0.5, procedural=0.4, problem_solving=0.6, communication=0.5, memory=0.3, spatial=0.0
    ))

    # Version 2A: Simple factors
    mcqs.append(kg.create_mcqs(
        ...
        correctindex=0,
        option_explanations=[
            'Correct! The numbers 2 and 3 multiply to give 6 and add to give 5, so \\((x + 2)(x + 3) = x^2 + 5x + 6\\).',
            'These factors give \\((x - 2)(x - 3) = x^2 - 5x + 6\\), which has the wrong middle term. We need +5x, not -5x.',
            'These factors give \\((x + 1)(x + 6) = x^2 + 7x + 6\\), which has +7x instead of +5x in the middle term.',
            'These factors give \\((x - 1)(x - 6) = x^2 - 7x + 6\\), which has both the wrong sign and wrong coefficient for the middle term.'
        ],
        main_topic_index=6,
        subtopic_weights={6: 0.9, 12: 0.1},
        conceptual=0.2, procedural=0.3, problem_solving=0.3, communication=0.2, memory=0.2, spatial=0.0
    ))

    # Version 2B: Medium difficulty
    mcqs.append(kg.create_mcqs(
        text='Factor the quadratic expression \\(x^2 - 5x + 6\\) by inspection.',
        options=[
            '\\((x - 2)(x - 3)\\)',
            '\\((x + 2)(x + 3)\\)',
            '\\((x - 1)(x - 6)\\)',
        ...
        correctindex=0,
        option_explanations=[
            'Correct! The numbers -2 and -3 multiply to +6 and add to -5, so \\((x - 2)(x - 3) = x^2 - 5x + 6\\).',
            'These factors give \\((x + 2)(x + 3) = x^2 + 5x + 6\\), which has +5x instead of -5x. When the constant is positive and the middle term is negative, both factors must be negative.',
            'These factors give \\((x - 1)(x - 6) = x^2 - 7x + 6\\), which has -7x instead of -5x in the middle term.',
            'These factors give \\((x + 1)(x + 6) = x^2 + 7x + 6\\), which has the wrong signs for both the middle term and doesn\'t match our target expression.'
        ],
        main_topic_index=6,
        subtopic_weights={6: 0.8, 12: 0.2},
        conceptual=0.3, procedural=0.4, problem_solving=0.4, communication=0.3, memory=0.3, spatial=0.0
    ))

    # Version 3A: Perfect square
    mcqs.append(kg.create_mcqs(
        text='Complete the square for \\(x^2 + 4x\\).',
        options=[
            '\\((x + 2)^2 - 4\\)',
            '\\((x + 2)^2 + 4\\)',
            '\\((x - 2)^2 - 4\\)',
        ...
        correctindex=0,
        option_explanations=[
            'Correct! Take half of 4 (which is 2), square it (which is 4), then \\(x^2 + 4x = x^2 + 4x + 4 - 4 = (x + 2)^2 - 4\\).',
            'This adds 4 instead of subtracting it. When completing the square, you must both add and subtract the same value to keep the expression equivalent.',
            'This uses (x - 2) instead of (x + 2). Since the original middle term is +4x, we need (x + 2) to get the +4x when expanded.',
            'This incorrectly uses 4 instead of half of 4. To complete the square for \\(x^2 + bx\\), use \\((\\frac{b}{2})^2\\), not \\(b^2\\).'
        ],
        main_topic_index=9,
        subtopic_weights={9: 0.8, 13: 0.15, 11: 0.05},
        conceptual=0.4, procedural=0.5, problem_solving=0.3, communication=0.3, memory=0.4, spatial=0.1
    ))

    # Version 4A: Direct reading
    mcqs.append(kg.create_mcqs(
        text='What is the vertex of \\(y = (x - 2)^2 + 3\\)?',
        options=[
            '\\((2, 3)\\)',
            '\\((-2, 3)\\)',
            '\\((2, -3)\\)',
        ...
        correctindex=0,
        option_explanations=[
            'Correct! In vertex form \\(y = (x - h)^2 + k\\), the vertex is (h, k). Here h = 2 and k = 3, so the vertex is (2, 3).',
            'This incorrectly uses h = -2. In the form (x - 2)², the value h = 2, not -2. The vertex form is (x - h), so (x - 2) means h = 2.',
            'This incorrectly uses k = -3. The +3 at the end means k = 3, not -3. The vertex form shows k directly as the constant term.',
            'This has both coordinates wrong. The vertex is (h, k) where h = 2 (from x - 2) and k = 3 (from +3), giving (2, 3).'
        ],
        main_topic_index=11,
        subtopic_weights={11: 0.7, 13: 0.25, 10: 0.05},
        conceptual=0.3, procedural=0.1, problem_solving=0.1, communication=0.3, memory=0.4, spatial=0.3
    ))

    # Version 5A: Simple values
    mcqs.append(kg.create_mcqs(
        text='Calculate the discriminant of \\(x^2 + 2x + 1 = 0\\).',
        options=[
            '\\(0\\)',
            '\\(4\\)',
            '\\(8\\)',
        ...
        correctindex=0,
        option_explanations=[
            'Correct! Using \\(\\Delta = b^2 - 4ac\\) with a=1, b=2, c=1: \\(\\Delta = 2^2 - 4(1)(1) = 4 - 4 = 0\\).',
            'This is just \\(b^2 = 2^2 = 4\\), but you forgot to subtract 4ac. The discriminant formula is \\(b^2 - 4ac\\), not just \\(b^2\\).',
            'This suggests you calculated \\(2b^2 = 2(4) = 8\\), but the discriminant formula uses \\(b^2\\) once, not twice.',
            'This uses the wrong sign. You might have calculated \\(4ac - b^2 = 4 - 4 = 0\\), but the discriminant is \\(b^2 - 4ac\\), not \\(4ac - b^2\\).'
        ],
        main_topic_index=17,
        subtopic_weights={17: 0.8, 16: 0.15, 15: 0.05},
        conceptual=0.2, procedural=0.4, problem_solving=0.1, communication=0.3, memory=0.5, spatial=0.0
    ))

    # Version 6A: Simple coefficients
    mcqs.append(kg.create_mcqs(
        text='Solve \\(x^2 - 3x + 2 = 0\\) using the quadratic formula.',
        options=[
            '\\(x = 1, 2\\)',
            '\\(x = -1, -2\\)',
            '\\(x = \\frac{3 \\pm 1}{2}\\)',
        ...
        correctindex=0,
        option_explanations=[
            'Correct! Using the quadratic formula: \\(x = \\frac{3 \\pm 1}{2}\\) gives \\(x = \\frac{4}{2} = 2\\) or \\(x = \\frac{2}{2} = 1\\).',
            'These are the negatives of the correct answers. Check your signs in the quadratic formula - with b = -3, you get -(-3) = +3 in the numerator.',
            'This shows the unsimplified form but doesn\'t complete the calculation. \\(\\frac{3 + 1}{2} = 2\\) and \\(\\frac{3 - 1}{2} = 1\\).',
            'This uses +3 instead of -3. When b = -3, the quadratic formula gives \\(\\frac{-(-3) \\pm \\sqrt{...}}{2a} = \\frac{3 \\pm \\sqrt{...}}{2a}\\).'
        ],
        main_topic_index=15,
        subtopic_weights={15: 0.6, 17: 0.2, 12: 0.15, 14: 0.05},
        conceptual=0.3, procedural=0.4, problem_solving=0.3, communication=0.4, memory=0.5, spatial=0.0
    ))

    # Version 7A: Direct discriminant given
    mcqs.append(kg.create_mcqs(
        text='If the discriminant of a quadratic equation is 16, what is the nature of its roots?',
        options=[
        ...
        correctindex=0,
        option_explanations=[
            'Correct! When the discriminant is positive (16 > 0), the quadratic equation has two distinct real roots.',
            'One repeated real root occurs when the discriminant equals zero, not when it\'s positive. A discriminant of 16 ≠ 0.',
            'Complex conjugate roots occur when the discriminant is negative. Since 16 > 0, the roots are real, not complex.',
            'The discriminant value alone is sufficient to determine the nature of roots. We can determine this from Δ = 16 > 0.'
        ],
        main_topic_index=16,
        subtopic_weights={16: 0.9, 17: 0.1},
        conceptual=0.4, procedural=0.1, problem_solving=0.1, communication=0.3, memory=0.4, spatial=0.0
    ))

    # Version 8A: Simple identification
    mcqs.append(kg.create_mcqs(
        text='Does \\(y = x^2 + 1\\) have any x-intercepts?',
        options=[
        ...
        correctindex=2,
        option_explanations=[
            'No, this would require y = 0 to have two solutions. Since \\(x^2 + 1 = 0\\) gives \\(x^2 = -1\\), which has no real solutions.',
            'No, this would require y = 0 to have exactly one solution. But \\(x^2 + 1\\) is always positive, never zero.',
            'Correct! Since \\(x^2 \\geq 0\\) for all real x, we have \\(y = x^2 + 1 \\geq 1 > 0\\). The parabola never crosses the x-axis.',
            'We can determine this by analyzing the equation. Since \\(x^2 + 1\\) is always positive, there are definitely no x-intercepts.'
        ],
        main_topic_index=10,
        subtopic_weights={10: 0.6, 12: 0.3, 16: 0.1},
        conceptual=0.4, procedural=0.1, problem_solving=0.2, communication=0.3, memory=0.2, spatial=0.4
    ))
    ...
    # QUESTION 11: Basic quadratic formula recall (Testing memory and notation)
    mcqs.append(kg.create_mcqs(
        text='What is the quadratic formula for solving \\(ax^2 + bx + c = 0\\)?',
        options=[
            '\\(x = \\frac{-b \\pm \\sqrt{b^2 - 4ac}}{2a}\\)',
            '\\(x = \\frac{b \\pm \\sqrt{b^2 - 4ac}}{2a}\\)',
            '\\(x = \\frac{-b \\pm \\sqrt{b^2 + 4ac}}{2a}\\)',
        ...
        correctindex=0,
        option_explanations=[
            'Correct! This is the standard quadratic formula: \\(x = \\frac{-b \\pm \\sqrt{b^2 - 4ac}}{2a}\\). The negative sign before b is crucial for the derivation.',
            'Missing the negative sign before b. The formula comes from completing the square where we get -b/2a, so the negative is essential.',
            'Wrong sign in the discriminant. It should be \\(b^2 - 4ac\\) (minus), not \\(b^2 + 4ac\\) (plus). This comes from the completing the square process.',
            'Missing the factor of 2 in the denominator. The denominator should be 2a, not just a, which comes from the coefficient when completing the square.'
        ],
        main_topic_index=15,  # 'using quadratic formula'
        subtopic_weights={15: 0.8, 14: 0.2},
        conceptual=0.2, procedural=0.1, problem_solving=0.1, communication=0.7, memory=0.8, spatial=0.0
    ))

    # QUESTION 12: Identifying when to use quadratic formula (Testing problem solving)
    mcqs.append(kg.create_mcqs(
        text='Which equation would be most efficiently solved using the quadratic formula?',
        options=[
            '\\(x^2 - 4 = 0\\)',
            '\\(x^2 + 6x + 9 = 0\\)',
            '\\(3x^2 + 7x - 1 = 0\\)',
        ...
        correctindex=2,
        option_explanations=[
            'This can be solved by taking square roots: \\(x^2 = 4\\), so \\(x = \\pm 2\\). No need for the quadratic formula.',
            'This is a perfect square: \\((x + 3)^2 = 0\\), so \\(x = -3\\). Factoring is simpler than the quadratic formula here.',
            'Correct! With a=3, b=7, c=-1, this doesn\'t factor easily. The quadratic formula is the most efficient method for this equation.',
            'This factors as \\(x(x - 5) = 0\\), giving \\(x = 0\\) or \\(x = 5\\). Factoring is much simpler than the quadratic formula.'
        ],
        main_topic_index=15,  # 'using quadratic formula'
        subtopic_weights={15: 0.6, 6: 0.25, 2: 0.15},
        conceptual=0.5, procedural=0.2, problem_solving=0.7, communication=0.3, memory=0.3, spatial=0.0
    ))

    # QUESTION 13: Word problem requiring quadratic formula (Testing problem solving and algebra)
    mcqs.append(kg.create_mcqs(
        text='A ball is thrown upward with initial velocity 20 m/s from a height of 1.5 m. Its height h (in meters) after t seconds is given by \\(h = -5t^2 + 20t + 1.5\\). When does the ball hit the ground?',
        options=[
            '\\(t = 4.1\\) seconds',
            '\\(t = 2.0\\) seconds',
            '\\(t = 3.5\\) seconds',
        ...
        correctindex=0,
        option_explanations=[
            'Correct! Setting h = 0 and using the quadratic formula: \\(t = \\frac{-20 \\pm \\sqrt{400 + 30}}{-10} \\approx \\frac{-20 \\pm 20.74}{-10}\\). The positive solution is t ≈ 4.1 seconds.',
            'This would be the time at maximum height (when derivative = 0), not when the ball hits the ground. You need to solve h = 0, not find the vertex.',
            'This appears to be a guess without proper calculation. The quadratic formula gives a different result when properly applied.',
            'Close but not accurate enough. Make sure to calculate \\(\\sqrt{430}\\) precisely and handle the negative denominator correctly.'
        ],
        main_topic_index=15,  # 'using quadratic formula'
        subtopic_weights={15: 0.7, 17: 0.2, 2: 0.1},
        conceptual=0.6, procedural=0.6, problem_solving=0.8, communication=0.4, memory=0.4, spatial=0.2
    ))

    # QUESTION 14: Complex roots using quadratic formula (Testing advanced concepts)
    mcqs.append(kg.create_mcqs(
        text='Find the solutions to \\(2x^2 + 3x + 2 = 0\\) using the quadratic formula.',
        options=[
        ...
        correctindex=0,
        option_explanations=[
            'Correct! With discriminant = 9 - 16 = -7 < 0, we get complex roots: \\(x = \\frac{-3 \\pm i\\sqrt{7}}{4}\\) where i = \\(\\sqrt{-1}\\).',
            'Wrong sign before 3. The quadratic formula gives \\(\\frac{-b}{2a} = \\frac{-3}{4}\\), so it should be -3, not +3 in the numerator.',
            'Missing the imaginary unit i. Since the discriminant is negative (-7), the square root of a negative number requires the imaginary unit: \\(\\sqrt{-7} = i\\sqrt{7}\\).',
            'While it\'s true there are no real solutions, the question asks for the solutions using the quadratic formula, which includes complex solutions.'
        ],
        main_topic_index=15,  # 'using quadratic formula'
        subtopic_weights={15: 0.6, 17: 0.25, 16: 0.15},
        conceptual=0.7, procedural=0.5, problem_solving=0.5, communication=0.8, memory=0.6, spatial=0.0
    ))

    # QUESTION 15: Deriving quadratic formula (Testing interconnected understanding)
    mcqs.append(kg.create_mcqs(
        text='In deriving the quadratic formula from \\(ax^2 + bx + c = 0\\), what is the result after completing the square?',
        options=[
            '\\((x + \\frac{b}{2a})^2 = \\frac{b^2 - 4ac}{4a^2}\\)',
            '\\((x - \\frac{b}{2a})^2 = \\frac{b^2 - 4ac}{4a^2}\\)',
            '\\((x + \\frac{b}{2a})^2 = \\frac{4ac - b^2}{4a^2}\\)',
            '\\((x + \\frac{b}{a})^2 = \\frac{b^2 - 4ac}{a^2}\\)'
        ],
        correctindex=0,
        option_explanations=[
            'Correct! Completing the square on \\(x^2 + \\frac{b}{a}x = -\\frac{c}{a}\\) gives \\((x + \\frac{b}{2a})^2 = (\\frac{b}{2a})^2 - \\frac{c}{a} = \\frac{b^2 - 4ac}{4a^2}\\).',
            'Wrong sign in the squared term. When completing the square for \\(x^2 + \\frac{b}{a}x\\), we get \\((x + \\frac{b}{2a})^2\\), not \\((x - \\frac{b}{2a})^2\\).',
            'Wrong sign in the right side. The discriminant should be \\(b^2 - 4ac\\), not \\(4ac - b^2\\). This comes from the algebra of completing the square.',
            'Wrong coefficient. After dividing by a, we get \\(\\frac{b}{a}\\), so half of this is \\(\\frac{b}{2a}\\), not \\(\\frac{b}{a}\\). Also, the denominator should be \\(4a^2\\).'
        ],
        main_topic_index=14,  # 'quadratic formula derivation'
        subtopic_weights={14: 0.7, 9: 0.2, 15: 0.1},
        conceptual=0.8, procedural=0.7, problem_solving=0.6, communication=0.7, memory=0.5, spatial=0.1
    ))

    # QUESTION 16: Discriminant and nature of roots (Testing memory and connections)
    mcqs.append(kg.create_mcqs(
        text='For what value of k does \\(x^2 + 6x + k = 0\\) have exactly one real solution?',
        options=[
            '\\(k = 9\\)',
            '\\(k = -9\\)',
            '\\(k = 3\\)',
        ...
        correctindex=0,
        option_explanations=[
            'Correct! For one real solution, discriminant = 0: \\(b^2 - 4ac = 36 - 4(1)(k) = 36 - 4k = 0\\), so k = 9.',
            'This would give discriminant = 36 - 4(-9) = 72 > 0, resulting in two distinct real solutions, not one.',
            'This gives discriminant = 36 - 12 = 24 > 0, which means two distinct real solutions, not exactly one.',
            'This gives discriminant = 36 - 144 = -108 < 0, which means no real solutions (complex solutions only).'
        ],
        main_topic_index=17,  # 'discriminant'
        subtopic_weights={17: 0.7, 16: 0.2, 15: 0.1},
        conceptual=0.6, procedural=0.4, problem_solving=0.5, communication=0.4, memory=0.6, spatial=0.0
    ))

    # QUESTION 17: Solving with fractions (Testing algebra skills)
    mcqs.append(kg.create_mcqs(
        text='Solve \\(\\frac{1}{2}x^2 - \\frac{3}{4}x - \\frac{1}{8} = 0\\) using the quadratic formula.',
        options=[
        ...
        correctindex=0,
        option_explanations=[
            'Correct! Using the quadratic formula carefully with fractions: discriminant = \\(\\frac{9}{16} + 4 \\cdot \\frac{1}{2} \\cdot \\frac{1}{8} = \\frac{9}{16} + \\frac{1}{4} = \\frac{17}{16}\\), giving \\(x = \\frac{3 \\pm \\sqrt{17}}{4}\\).',
            'Incorrect discriminant calculation. Make sure to compute \\(b^2 - 4ac = \\frac{9}{16} + \\frac{1}{4} = \\frac{17}{16}\\), not 13/16.',
            'Wrong sign. Since b = -3/4, we have -b = 3/4, so the numerator should start with +3, not -3.',
            'These might be correct values, but they\'re not in the form the quadratic formula produces. Always show your work with the formula.'
        ],
        main_topic_index=15,  # 'using quadratic formula'
        subtopic_weights={15: 0.8, 17: 0.15, 0: 0.05},
        conceptual=0.4, procedural=0.8, problem_solving=0.4, communication=0.6, memory=0.3, spatial=0.0
    ))

    # QUESTION 18: Applications - finding intersection points (Testing interconnected skills)
    mcqs.append(kg.create_mcqs(
        text='Find the x-coordinates where the parabola \\(y = x^2 - 3x + 1\\) intersects the line \\(y = 2x - 1\\).',
        options=[
        ...
        correctindex=0,
        option_explanations=[
            'Correct! Setting \\(x^2 - 3x + 1 = 2x - 1\\) gives \\(x^2 - 5x + 2 = 0\\). Using the quadratic formula: \\(x = \\frac{5 \\pm \\sqrt{25 - 8}}{2} = \\frac{5 \\pm \\sqrt{17}}{2}\\).',
            'This suggests you used the wrong coefficients. Check that you correctly rearranged to \\(x^2 - 5x + 2 = 0\\), not \\(x^2 - 3x + 2 = 0\\).',
            'Incorrect discriminant calculation. With \\(x^2 - 5x + 2 = 0\\), the discriminant is \\(25 - 8 = 17\\), not 13.',
            'The discriminant is positive (17 > 0), so there are two intersection points. Check your algebra in setting up the equation.'
        ],
        main_topic_index=15,  # 'using quadratic formula'
        subtopic_weights={15: 0.5, 12: 0.3, 10: 0.2},
        conceptual=0.6, procedural=0.6, problem_solving=0.7, communication=0.4, memory=0.3, spatial=0.4
    ))

    # QUESTION 19: Quadratic formula with parametric coefficients (Testing notation and algebra)
    mcqs.append(kg.create_mcqs(
        text='For the equation \\(px^2 + qx + r = 0\\) where \\(p \\neq 0\\), what is the sum of the roots using the quadratic formula?',
        options=[
            '\\(-\\frac{q}{p}\\)',
            '\\(\\frac{q}{p}\\)',
            '\\(-\\frac{r}{p}\\)',
        ...
        correctindex=0,
        option_explanations=[
            'Correct! Adding the two roots: \\(\\frac{-q + \\sqrt{q^2 - 4pr}}{2p} + \\frac{-q - \\sqrt{q^2 - 4pr}}{2p} = \\frac{-2q}{2p} = -\\frac{q}{p}\\). This is also Vieta\'s formula.',
            'Missing the negative sign. The sum of roots is \\(-\\frac{q}{p}\\), not \\(+\\frac{q}{p}\\). This comes from the -q term in the quadratic formula.',
            'This is the product of the roots (\\(\\frac{r}{p}\\)), not the sum. The sum involves the coefficient of x, which is q.',
            'This looks like you\'re trying to use the discriminant, but the sum of roots is simply the coefficient relationship, not involving the discriminant.'
        ],
        main_topic_index=15,  # 'using quadratic formula'
        subtopic_weights={15: 0.6, 14: 0.3, 2: 0.1},
        conceptual=0.7, procedural=0.6, problem_solving=0.6, communication=0.8, memory=0.4, spatial=0.0
    ))

    # QUESTION 20: Error analysis in quadratic formula application (Testing understanding and problem solving)
    mcqs.append(kg.create_mcqs(
        text='A student solving \\(3x^2 - 2x - 1 = 0\\) wrote \\(x = \\frac{2 \\pm \\sqrt{4 + 12}}{6}\\). What error did they make?',
        options=[
            'They used the wrong sign for the discriminant',
            'They forgot the negative sign before b',
            'They used the wrong values for a, b, or c',
            'No error - this is correct'
        ],
        correctindex=3,
        option_explanations=[
            'The discriminant \\(b^2 - 4ac = (-2)^2 - 4(3)(-1) = 4 + 12 = 16\\) is calculated correctly with the right sign.',
            'The coefficient b = -2, so -b = -(-2) = +2. The student correctly wrote +2 in the numerator.',
            'The student correctly identified a = 3, b = -2, c = -1, and used these values properly in the formula.',
            'Correct! The student applied the quadratic formula correctly: \\(x = \\frac{-(-2) \\pm \\sqrt{(-2)^2 - 4(3)(-1)}}{2(3)} = \\frac{2 \\pm \\sqrt{16}}{6} = \\frac{2 \\pm 4}{6}\\), giving x = 1 or x = -1/3.'
        ],
        main_topic_index=15,  # 'using quadratic formula'
        subtopic_weights={15: 0.7, 17: 0.2, 2: 0.1},
        conceptual=0.6, procedural=0.4, problem_solving=0.7, communication=0.5, memory=0.4, spatial=0.0
    ))
    # MCQ 1: Linear equations in standard form (Topic 1)
    # This topic only appears in subtopic weights, needs dedicated coverage
    mcqs.append(kg.create_mcqs(
        text='Which of the following equations is written in standard form?',
        options=[
            '2x = 8 - 3y',
            '4x + 5y = 12',
            'y = 2x + 7',
        ...
        correctindex=1,
        option_explanations=[
            'This equation has variables on both sides. Standard form requires all variables on the left side: Ax + By = C.',
            'Correct! This is in standard form Ax + By = C where A = 4, B = 5, and C = 12. All variables are on the left, constant on the right.',
            'This is slope-intercept form (y = mx + b), not standard form. Standard form has both variables on the same side.',
            'This has the constant term on the wrong side. Standard form requires the constant to be positive on the right: x - y = -3.'
        ],
        main_topic_index=1,  # 'linear equations in standard form'
        subtopic_weights={1: 0.8, 0: 0.2},  # mainly standard form, some solving skills
        conceptual=0.4, procedural=0.6, problem_solving=0.3, communication=0.4, memory=0.5, spatial=0.0
    ))

    # MCQ 2: Expanding brackets (Topic 3)
    # Core algebraic skill that's prerequisite to many other topics
    mcqs.append(kg.create_mcqs(
        text='Expand and simplify: 3(2x - 4) + 2(x + 5)',
        options=[
            '6x - 12 + 2x + 10 = 8x - 2',
            '5x + 1',
            '8x + 2',
        ...
        correctindex=0,
        option_explanations=[
            'Correct! First expand each bracket: 3(2x - 4) = 6x - 12, and 2(x + 5) = 2x + 10. Then combine: 6x - 12 + 2x + 10 = 8x - 2.',
            'This incorrectly combines unlike terms. You cannot add 3(2x - 4) and 2(x + 5) directly without expanding first.',
            'This gets the x-coefficient right (8x) but the constant term wrong. -12 + 10 = -2, not +2.',
            'This misses terms from the expansion. The correct expansion gives 8x - 2, not 6x - 2.'
        ],
        main_topic_index=3,  # 'expanding brackets'
        subtopic_weights={3: 0.7, 0: 0.3},  # mainly expanding, some basic algebra
        conceptual=0.3, procedural=0.8, problem_solving=0.4, communication=0.3, memory=0.4, spatial=0.0
    ))

    # MCQ 3: Substitution (Topic 4)
    # Important skill for solving systems and evaluation
    mcqs.append(kg.create_mcqs(
        text='If a = 3 and b = -2, find the value of 2a² - 3ab + b².',
        options=[
            '18 + 18 + 4 = 40',
            '18 - 18 + 4 = 4',
            '6 + 6 + 4 = 16',
        ...
        correctindex=0,
        option_explanations=[
            'Correct! Substitute a = 3, b = -2: 2(3)² - 3(3)(-2) + (-2)² = 2(9) - 3(3)(-2) + 4 = 18 - (-18) + 4 = 18 + 18 + 4 = 40.',
            'This correctly calculates 2a² = 18 and b² = 4, but makes an error with -3ab. When a = 3 and b = -2, -3ab = -3(3)(-2) = +18, not -18.',
            'This uses a = 3 incorrectly in 2a². It should be 2(3)² = 2(9) = 18, not 2(3) = 6.',
            'This has multiple errors: wrong calculation of a² and incorrect handling of the negative signs in the substitution.'
        ],
        main_topic_index=4,  # 'substitution'
        subtopic_weights={4: 0.9, 3: 0.1},  # mainly substitution, minimal expanding
        conceptual=0.4, procedural=0.7, problem_solving=0.5, communication=0.4, memory=0.3, spatial=0.0
    ))

    # MCQ 4: Basic coordinate geometry (Topic 8)
    # Fundamental geometry skill that connects algebra and geometry
    mcqs.append(kg.create_mcqs(
        text='Find the distance between points A(1, 2) and B(4, 6).',
        options=[
            '5',
            '7',
            '√7',
        ...
        correctindex=0,
        option_explanations=[
            'Correct! Using the distance formula: d = √[(x₂-x₁)² + (y₂-y₁)²] = √[(4-1)² + (6-2)²] = √[3² + 4²] = √[9 + 16] = √25 = 5.',
            'This is the sum of the coordinate differences: |4-1| + |6-2| = 3 + 4 = 7. This is not the straight-line distance.',
            'This incorrectly calculates the distance as √[(4-1) + (6-2)] = √[3 + 4] = √7, forgetting to square the differences.',
            'This correctly calculates √25 but doesn\'t simplify. √25 = 5 is the final answer.'
        ],
        main_topic_index=8,  # 'basic coordinate geometry'
        subtopic_weights={8: 1.0},  # purely coordinate geometry
        conceptual=0.5, procedural=0.6, problem_solving=0.5, communication=0.4, memory=0.4, spatial=0.7
    ))

    # MCQ 5: Vertex of parabola (Topic 11)
    # Important endpoint skill that needs more coverage
    mcqs.append(kg.create_mcqs(
        text='What is the vertex of the parabola y = 2(x - 3)² + 1?',
        options=[
            '(3, 1)',
            '(-3, 1)',
            '(3, -1)',
        ...
        correctindex=0,
        option_explanations=[
            'Correct! In the form y = a(x - h)² + k, the vertex is at (h, k). Here h = 3 and k = 1, so the vertex is (3, 1).',
            'This uses the wrong sign for the x-coordinate. The form is (x - h), so when we have (x - 3), h = +3, not -3.',
            'This uses the wrong sign for the y-coordinate. The k value is +1, giving vertex (3, 1), not (3, -1).',
            'This has both coordinates wrong. From y = 2(x - 3)² + 1, the vertex is (3, 1), not (-3, -1).'
        ],
        main_topic_index=11,  # 'vertex of parabola'
        subtopic_weights={11: 0.8, 9: 0.2},  # mainly vertex, some completed square form
        conceptual=0.6, procedural=0.5, problem_solving=0.3, communication=0.4, memory=0.4, spatial=0.6
    ))

    # MCQ 6: Transformations (Topic 12)
    # Important graphical concept that needs coverage
    mcqs.append(kg.create_mcqs(
        text='The graph of y = x² is transformed to y = (x + 2)² - 3. Describe this transformation.',
        options=[
            'Left 2 units, down 3 units',
            'Right 2 units, down 3 units',
            'Left 2 units, up 3 units',
            'Right 2 units, up 3 units'
        ],
        correctindex=0,
        option_explanations=[
            'Correct! y = (x + 2)² moves the graph left 2 units (opposite of the sign inside), and -3 moves it down 3 units.',
            'The horizontal shift is wrong. (x + 2) means move left 2 units, not right. The vertical shift down 3 is correct.',
            'The vertical shift is wrong. The -3 means move down 3 units, not up. The horizontal shift left 2 is correct.',
            'Both shifts are wrong. (x + 2) means left 2 units, and -3 means down 3 units.'
        ],
        main_topic_index=12,  # 'transformations'
        subtopic_weights={12: 0.9, 10: 0.1},  # mainly transformations, some graph knowledge
        conceptual=0.7, procedural=0.4, problem_solving=0.4, communication=0.5, memory=0.3, spatial=0.8
    ))
...
def test_greedy_algorithm_functionality():
    ...
    try:
        # Initialize system with specific config for testing
        test_config = AlgorithmConfig(
            mastery_threshold=0.7,
            greedy_priority_weight=2.0,
            greedy_subtopic_weight=0.7,
            greedy_prereq_weight=0.5,
            greedy_mastery_update_rate=0.8,
            greedy_early_stopping=False,  # Disable for predictable testing
            greedy_max_mcqs_to_evaluate=50
        )

        kg = KnowledgeGraph()
        student_manager = StudentManager(test_config)
        mcq_scheduler = MCQScheduler(kg, student_manager, test_config)
        bkt_system = BayesianKnowledgeTracing(kg, student_manager, test_config)
        ...
        display_student_mastery_summary(student, kg, test_config)
        ...
        print("-" * 40)
        test_edge_cases(mcq_scheduler, student_manager, kg, bkt_system)
...
def create_test_student_with_varied_mastery(student_manager, kg, bkt_system, student_id):
    ...
    mastery_distribution = {
        ...
        'low': (0.3, 0.5),         # Due topics
        'medium': (0.5, 0.7),      # Due topics (just below threshold)
        'high': (0.7, 0.9),        # Not due (above threshold)
        'very_high': (0.9, 1.0)    # Not due
    }
...
def display_student_mastery_summary(student, kg, config):
    ...
    print(f"🔴 Due Topics (below {config.mastery_threshold}): {len(due_topics)}")
    print(f"🟢 Not Due Topics (above {config.mastery_threshold}): {len(not_due_topics)}")
...
def test_topic_priority_calculation(mcq_scheduler, student_id, kg):
    ...
    for main_topic_index, priority in topic_priorities.items():
        mastery = student.get_mastery(main_topic_index)
        if mastery >= mcq_scheduler.config.mastery_threshold:
...
def test_edge_cases(mcq_scheduler, student_manager, kg, bkt_system):
    """Test edge cases and error handling"""
    print("🔍 Testing edge cases...")

    # Test 1: Student with no due topics
    print("\n1. Student with all high mastery (no due topics):")
    high_mastery_student = student_manager.create_student("high_mastery_test")

    # Set all topics to high mastery
    for main_topic_index in kg.get_all_indexes()[:10]:  # Limit for testing
        high_mastery_student.mastery_levels[main_topic_index] = 0.9
        high_mastery_student.studied_topics[main_topic_index] = True

    high_mastery_mcqs = mcq_scheduler.select_mcqs_greedy("high_mastery_test", 3)
    print(f"   Selected MCQs: {len(high_mastery_mcqs)} (should fallback to exploration)")

    # Test 2: Student with no studied topics
    print("\n2. Student with no studied topics:")
    no_study_student = student_manager.create_student("no_study_test")

    # Set low mastery but no studied topics
    for main_topic_index in kg.get_all_indexes()[:5]:
        no_study_student.mastery_levels[main_topic_index] = 0.2
        no_study_student.studied_topics[main_topic_index] = False  # Not studied

    no_study_mcqs = mcq_scheduler.select_mcqs_greedy("no_study_test", 3)
    print(f"   Selected MCQs: {len(no_study_mcqs)} (should be 0 - no eligible MCQs)")

    # Test 3: Student with all questions completed today
    print("\n3. Student with all questions completed today:")
    completed_student = student_manager.create_student("completed_test")

    # Set up student with due topics
    for main_topic_index in kg.get_all_indexes()[:5]:
        completed_student.mastery_levels[main_topic_index] = 0.3
        completed_student.studied_topics[main_topic_index] = True

    # Mark all MCQs as completed today
    all_mcqs = list(kg.mcqs.keys())
    completed_student.daily_completed.update(all_mcqs)

    completed_mcqs = mcq_scheduler.select_mcqs_greedy("completed_test", 3)
    print(f"   Selected MCQs: {len(completed_mcqs)} (should be 0 - all completed today)")
